...
def explosion_radius_single_bomb(coordinates):
	# arena = np.array(self.game_state['arena'])
	check_walls = check_even_odd(coordinates)[np.r_[1, 0]]
	x, y, x_, y_ = *coordinates, *check_walls
	row = [[item, y] for item in np.unique(np.clip(np.r_[x-3:x+4], 0, 17))] if not x_ else [[x, y]]
	column = [[x, item] for item in np.unique(np.clip(np.r_[y-3:y+4], 0, 17))] if not y_ else [[x, y]]
	explosion_tiles = np.unique(np.concatenate((row, column)), axis = 0)
