...
def next_move_blocked (self):
    ...
    for i in range(4):
        blocked[i] = -3 + np.clip(np.sum(directions_blocked(own_pos + pos_diffs[i], self)), 3, 4)
    return blocked
...
def reward_update_(self):
    ...
    event_dict = {0 : "MOVED_LEFT",
            ...
            16 : "SURVIVED_ROUND"}
    f = features
    rewards = {"MOVED_LEFT" : -2 + f[1] + 0.2*f[3] + 0.2*f[5] - 3*f[7] - 2*f[18] + 0.3*f[22] + 0.1*f[24] - 10*f[27], 
                    "MOVED_RIGHT" : -2 - f[1] - 0.2*f[3] + 0.2*f[5] - 3*f[8] - 2*f[19] - 0.3*f[22] - 0.1*f[24] - 10*f[28],
                    "MOVED_UP" : -2 + f[2] + 0.2*f[4] + 0.2*f[6] - 3*f[9] - 2*f[20] + 0.3*f[23] + 0.1*f[25] - 10*f[29],
                    "MOVED_DOWN" : -2 - f[2] - 0.2*f[4] + 0.2*f[6] - 3*f[10] - 2*f[21] - 0.3*f[23] - 0.1*f[25] - 10*f[30],
                    "WAITED" : -6 - 40*f[11], 
                    "INTERRUPTED" : -1,
