...
def directions_blocked(pos, self):# self):
    pos = np.array(pos)
...
def build_features (self):
    ...
    features = np.append(features, next_move_blocked(self)) # 4 werte, indizes 27, 28, 29, 30
    return features
...
def act(self):  
    try:
        ...
        # if len(np.array(self.game_state['bombs'])) > 0 or (np.array(self.game_state['explosions']) > 0).any(): 
        #     print('self: ', self.game_state['self'])
        #     print('features: ', features[18], features[19], features[20], features[21])
        #print('theta')
        #print(theta_q)
        #print('features')
        # print('q_values')
...
def reward_update_(self):
    ...
    event_dict = {0 : "MOVED_LEFT",
            ...
            16 : "SURVIVED_ROUND"}
    f = features
    rewards = {"MOVED_LEFT" : -2 + f[1] + 0.2*f[3] + 0.2*f[5] - 3*f[7] - 3*f[18] + 0.5*f[22] + 0.15*f[24] - 14*f[27], 
                    "MOVED_RIGHT" : -2 - f[1] - 0.2*f[3] + 0.2*f[5] - 3*f[8] - 3*f[19] - 0.5*f[22] - 0.15*f[24] - 14*f[28],
                    "MOVED_UP" : -2 + f[2] + 0.2*f[4] + 0.2*f[6] - 3*f[9] - 3*f[20] + 0.5*f[23] + 0.15*f[25] - 14*f[29],
                    "MOVED_DOWN" : -2 - f[2] - 0.2*f[4] + 0.2*f[6] - 3*f[10] - 3*f[21] - 0.5*f[23] - 0.15*f[25] - 14*f[30],
                    "WAITED" : -2 - 40*f[11], 
                    "INTERRUPTED" : -1,
...
def end_of_episode(self):
    try:
        ...
        for t in range(len(moves)):
            ...
            # print('Q_n: ', y_t)
            # print('old_value: ', history[-len(moves)+ t, 1])
            # print('diff: ', q_update - history[-len(moves) + t, 1])
        ...
        if len(all_data) > 30000:
            all_data = all_data[np.array(random.sample(list(np.arange(len(all_data))), 30000))]    
        np.save('agent_code/qn_agent/all_data.npy', all_data)
        # print(self.events)    
        #print('END')
        moves = np.array([[]])    
