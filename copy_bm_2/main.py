...
def main():
    ...
    with open('{}round_number.txt'.format(path), 'r') as f:
        start_round_number = int(f.read())
        print('Beginn bei Rundenzahl:', start_round_number) 

    # Run one or more games
    for i in range(s.n_rounds):
        if ((i+start_round_number) % 20 == 0):
            print(i+start_round_number)
            theta_save = np.load('{}theta_q.npy'.format(path))
            np.save('{}theta_values/theta_alte_params_nach_{}_spielen.npy'.format(path, i+start_round_number), theta_save)
